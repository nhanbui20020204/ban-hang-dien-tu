...
{
    public function storeAdmin(CreateSanPhamRequest $request)
    {
        // dd($request->all());
        $admin   = Auth::guard('admin')->user(); // lấy thông tin của tài khoản đang đăng nhập

        $data                   = $request->all();
        $data['id_admin']     = $admin->id;
        SanPham::create($data);
    ...
    public function search(Request $request)
    {
        ...
        $data    =  SanPham::where('id_partner', 'like',$partner->id)
    ...
    public function statusAdmin(ChangeStatusSanPhamRequest $request)
    {
        $admin   = Auth::guard('admin')->user(); // lấy thông tin của tài khoản đang đăng nhập
        $san_pham = SanPham::where('id', $request->id)
    ...
    {
        return view('admin.san_pham.index_admin');
    }
